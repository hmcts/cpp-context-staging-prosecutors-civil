...
public class SubmissionErrorDetailsCsvResponseStrategyTest {
    ...
    public void shouldReturnCsvEntityWithTextCsvContentType() {
        ...
        final Response response = responseStrategy.responseFor("stagingprosecutorscivil.submission-error-details", Optional.of(envelope));
    ...
    @Test
    public void shouldUseOriginalFileNameWithErrorSuffixWhenFileNamePresent() {
        ...
        final JsonEnvelope envelope = EnvelopeFactory.createEnvelope("stagingprosecutorscivil.query.submission-error-details-csv",
                createObjectBuilder()
                        .add("submissionId", submissionId)
                        .add("csv", csv)
                        .add("fileName", "complaints-2026-01-01.csv")
                        .build());

        final Response response = responseStrategy.responseFor("stagingprosecutorscivil.submission-error-details", Optional.of(envelope));

        assertThat(response.getHeaderString(HttpHeaders.CONTENT_DISPOSITION),
                containsString("filename=\"complaints-2026-01-01_error.csv\""));
    }

    @Test
    public void shouldReturnOkWithNoEntityWhenEnvelopeAbsent() {
        final Response response = responseStrategy.responseFor("stagingprosecutorscivil.submission-error-details", Optional.empty());
