...
public class DefaultQueryApiSubmissionsSubmissionIdResourceTest {
    ...
    public void shouldUseCsvResponseStrategyForCsvAction() {
        final String submissionId = UUID.randomUUID().toString();
        when(actionMapper.actionOf("getSubmissionsBySubmissionId", "GET", headers))
                .thenReturn("stagingprosecutorscivil.submission-error-details-csv");
        when(restProcessor.process(eq("SubmissionErrorDetailsCsvResponseStrategy"), any(), eq("stagingprosecutorscivil.submission-error-details-csv"), eq(headers), anyCollection()))
                .thenReturn(expectedResponse);

        final Response response = resource.getSubmissionsBySubmissionId(submissionId, null);

        assertThat(response, is(expectedResponse));
        verify(restProcessor).process(eq("SubmissionErrorDetailsCsvResponseStrategy"), any(), eq("stagingprosecutorscivil.submission-error-details-csv"), eq(headers), anyCollection());
    }
}
