...
import static uk.gov.moj.cpp.staging.prosecutors.civil.util.StagingProsecutorsCivilUtils.ENFORCEMENT_PROSECUTION_CONTENT_TYPE;
...
public class EnforcementProsecutionIT {
    ...
    public void shouldSubmitProsecutionForGroupCaseSuccess() {
        ...
        UrlResponse urlResponse = StagingProsecutorsCivilUtils.submitEnforcementProsecution("payload/enforcement/stagingprosecutors.submit-enforcement-prosecution-all-fields.json", ENFORCEMENT_PROSECUTION_CONTENT_TYPE);
        final UUID submissionId = urlResponse.getSubmissionId();
        ProsecutionCaseFileApi.expectInitiateGroupProsecutionInvokedWith("payload/enforcement/stagingprosecutors.submit-enforcement-prosecution-all-fields.json");
    ...
    public void shouldSubmitProsecutionForGroupCaseSuccessWithOnlyMandatoryFields() {
        stubPCFCommand(randomUUID());
        UrlResponse urlResponse = StagingProsecutorsCivilUtils.submitEnforcementProsecution("payload/enforcement/stagingprosecutors.submit-enforcement-prosecution-mandatory-fields-only.json", ENFORCEMENT_PROSECUTION_CONTENT_TYPE);
        ProsecutionCaseFileApi.expectInitiateGroupProsecutionInvokedWith("payload/enforcement/stagingprosecutors.submit-enforcement-prosecution-all-fields.json");
    ...
    public void shouldSubmitProsecutionForSingleCaseSuccess() {
        stubPCFCommand(randomUUID());
        UrlResponse urlResponse = StagingProsecutorsCivilUtils.submitEnforcementProsecution("payload/enforcement/stagingprosecutors.submit-enforcement-prosecution-single-case.json", ENFORCEMENT_PROSECUTION_CONTENT_TYPE);
        final UUID submissionId = urlResponse.getSubmissionId();
        ProsecutionCaseFileApi.expectInitiateSingleProsecution("payload/enforcement/stagingprosecutors.submit-enforcement-prosecution-single-case.json");
        final Submission submission = StagingProsecutorsCivilUtils.pollForSubmission(submissionId, SubmissionStatus.PENDING);
        assertThat(submission.getSubmissionId().toString(), Matchers.is(urlResponse.getSubmissionId().toString()));
